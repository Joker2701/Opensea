...
class ScanConfig:
    ...
    top_n: int = 15
    # НЕВІДКАЛІБРОВАНО ПРОТИ ОФІЦІЙНОЇ ДОКУМЕНТАЦІЇ: коефіцієнт параболічної
    # комісії Polymarket за маркет-ноги (a·P·(1−P), максимум = a/4 при P=0,5).
    # 0.072 -> 1.8% при P=0,5 — узято з ОДНОГО числового прикладу в
    # сторонньому Discord-гайді користувача, а не з офіційних docs.polymarket.com
    # (мережа до них у середовищі збірки закрита). Якщо ви бачите реальну
    # комісію в акаунті чи офіційному джерелі — поправте тут, код нічого не
    # хардкодить далі. Див. docs/API_NOTES.md.
    polymarket_taker_fee_coeff: float = 0.072
