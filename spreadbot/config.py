...
class ScanConfig:
    ...
    max_markets: int = 200
    strike_ratios: list[float] = field(default_factory=lambda: [0.6, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.05, 1.1, 1.2, 1.3, 1.4])
    ...
    top_n: int = 15
    # Коефіцієнт параболічної комісії Polymarket за маркет-ноги крипто-ринків
    # (a·P·(1−P), максимум = a/4 при P=0,5 -> 0.07 дає $1.75/100 шейрів).
    # Пряма мережа до docs.polymarket.com в середовищі збірки закрита, але
    # значення перехресно підтверджене через WebSearch кількома незалежними
    # джерелами (Fee Schedule V2, крипто-категорія, чинна з 30.03.2026) —
    # див. коментар у strategy/costs.py. Якщо побачите інше число в
    # акаунті чи офіційному джерелі — поправте тут, код нічого не хардкодить.
    polymarket_taker_fee_coeff: float = 0.07
